...
pull_package_apks() {
  ...
  # tr -d '\r' strips Windows-style line endings from adb output
  apk_paths=()
  while IFS= read -r line; do
    [ -n "$line" ] && apk_paths+=("$line")
  done < <(adb shell pm path "$package_name" | sed 's/package://' | tr -d '\r')
  ...
  if [ $apk_count -eq 1 ]; then
    ...
    # Bug fix: $apk_path is the device path; the local destination is $output_dir/
    _success "APK saved to: $output_dir/"
  else
    ...
    # Bug fix: was $app_name (undefined); use $package_name
    merge_apks "$output_dir" "$package_name"
  fi
}
...
pull_apks() {
  ...
  done < <(adb shell pm list packages -f "$app" | sed 's/package://' | tr -d '\r')
